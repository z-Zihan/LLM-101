...
从[第一篇：AI 到底是什么？](./docs/01-AI与大模型/01-AI到底是什么.md)开始即可。每篇文章顶部会说明你在哪里，结尾会给出自然的下一步。完整的 1～32 步顺序清单按真实教学顺序列在[知识网络的主学习路线](./知识网络.md#主学习路线)，迷路时回到那里就能恢复位置。

说明一点：文件名中的编号反映写作与合并的历史，不代表阅读顺序；阅读顺序始终以每篇文章顶部的课程导航为准。
...
- 历史专题（3 篇）：从[规则系统到深度学习](./history/01-从规则系统到深度学习.md)读起，交代每个技术为什么在那个年代出现

## 这套教程怎样控制质量

每篇文章不追求塞进所有相关知识，而要让读者完成一个清晰的认知任务。关键事实由原始论文、官方规范、官方文档或权威教材支撑；具体问题只用来发现读者会卡在哪里。

发布前会检查准确性、零基础可读性、内容减法、问题覆盖、术语、排版、链接、叙事连续性、下一步路由和问题隐私。内容质量的最终标准是真实零基础读者能否顺着读下去：开展读者测试时只记录匿名编号和可观察行为，不记录可识别身份，未执行的测试不补写结果；多名读者在同一位置卡住时，修订对应入口或文章后复测。
...
项目范围与编辑规则放在二级入口：[内容地图](./内容地图.md) · [文章模板](./文章模板.md) · [排版规范](./排版规范.md) · [术语表](./术语表.md)。
