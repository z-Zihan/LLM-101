...
> 当前发布范围已经闭环：62 篇已发布文章达到当前标准，174 个已入库真实问题全部有答案；长期路线图 B1 已完成，仍有 Deferred / Verify 项，详见下方边界说明。

## 当前完成度与边界

| 维度 | 当前状态 |
|---|---|
| 已发布正文 | 62 篇；其中 51 篇原有正文完成 V3 重写，新增 11 篇扩展正文 |
| 真实问题 | 174 条已入库，174 条 `answered`，High 未回答为 0 |
| 知识网络 | 53 个节点、82 条语义关系 |
| 内容地图 | 141 个覆盖项 `Done`，38 个长期选题 `Deferred`，3 个易变产品项 `Verify` |
...
- 19 个尚缺专项解释的内容选题，例如模型文件格式、安全红队、独立工具类型、Copilot 与 Agent、知识图谱对比；
...
Deferred 与 Verify 已整理成[长期内容路线图](./长期内容路线图.md)：规划为 Now / Next / Later 共 9 个批次、约 27 个实际交付物；B1 的 3 篇已完成，剩余 24 个计划交付物。每批仍按“研究 → 重写 → 真实问题与知识网络更新 → 15 项审查 → 自动检查 → 提交推送”的合同推进。
