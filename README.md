...
> 当前发布范围已经闭环：71 篇已发布文章与图谱资产达到当前标准，186 个已入库真实问题全部有答案；长期路线图 B1～B4 已完成，仍有 Deferred / Verify 项，详见下方边界说明。

## 当前完成度与边界

| 维度 | 当前状态 |
|---|---|
| 已发布正文与图谱资产 | 71 篇；其中 51 篇原有正文完成 V3 重写，新增 20 篇扩展正文与图谱资产 |
| 真实问题 | 186 条已入库，186 条 `answered`，High 未回答为 0 |
| 知识网络 | 65 个节点、108 条语义关系 |
| 内容地图 | 157 个覆盖项 `Done`，22 个长期选题 `Deferred`，3 个易变产品项 `Verify` |
...
Deferred 与 Verify 已整理成[长期内容路线图](./长期内容路线图.md)：规划为 Now / Next / Later 共 9 个批次、约 27 个实际交付物；B1～B4 共 12 个交付物已完成，剩余约 15 个。每批仍按“研究 → 重写 → 真实问题与知识网络更新 → 15 项审查 → 自动检查 → 提交推送”的合同推进。
