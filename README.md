...
> 当前发布范围已经闭环：59 篇已发布文章达到当前标准，168 个已入库真实问题全部有答案；第二轮 78 条候选均有逐题审计结论。长期内容地图仍有 Deferred / Verify 项，详见下方边界说明。

## 当前完成度与边界

| 维度 | 当前状态 |
|---|---|
| 已发布正文 | 59 篇；其中 51 篇原有正文完成 V3 重写，新增 8 篇扩展正文 |
| 真实问题 | 168 条已入库，168 条 `answered`，High 未回答为 0 |
| 知识网络 | 47 个节点、73 条语义关系 |
| 内容地图 | 135 个覆盖项 `Done`，44 个长期选题 `Deferred`，3 个易变产品项 `Verify` |
...
- 25 个尚缺专项解释的内容选题，例如 System Prompt、模型文件格式、安全红队、独立工具类型、Copilot 与 Agent、知识图谱对比；
...
Deferred 与 Verify 已整理成[长期内容路线图](./长期内容路线图.md)：规划为 Now / Next / Later 共 9 个批次、约 27 个实际交付物。每批仍按“研究 → 重写 → 真实问题与知识网络更新 → 15 项审查 → 自动检查 → 提交推送”的合同推进。
