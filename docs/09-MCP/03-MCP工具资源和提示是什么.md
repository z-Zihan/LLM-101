...
规则只有在自己动手分类时才会变成直觉。下面四个场景值得先自己想一遍再看答案。

“列出字体库里有哪些可用字体”——读取既存内容，不产生变化，是 Resource。“把系统切换到深色主题”——改变外部状态，哪怕再小也是 Tool，因为关键在副作用而非数据量。“新成员入职引导的标准话术”——按需取用的消息模板，用户选了才生效，是 Prompt。最有意思的是“查询库存”：如果只是读出数字，Resource 就够；如果要支持“预占十件”这类带业务后果的动作，就得做成 Tool。同一个业务对象落在哪一类，取决于交互的意图，而不是它叫什么名字。

现实实现里还允许组合：一次工具调用返回的数据里附带可读的资源引用，或者模板参数填完后触发调用。分类清楚的回报在于，Host 可以对三类能力分别设计确认与授权，而不是一刀切。
