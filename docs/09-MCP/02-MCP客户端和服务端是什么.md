...
分层的好处这时就体现了：故障可以按层排查，而不是整个系统打问号。

先看 Transport——进程存在吗？本地子进程是不是启动即退出了？远端地址通不通？这一层失败的特征是消息根本没到达对方。Transport 正常再看初始化：版本协商或能力声明的报错说明两端能通信，但对“说什么”没有达成一致，常见于升级后只改了一端。接着查发现：握手成功却列不出工具，多半是 Server 实现了协议却没有正确暴露能力。最后才轮到语义层——Tool 被正常调用但结果错误，那已经进入业务逻辑和权限配置的范围，协议侧无能为力。

按这个顺序走一遍，“连接不上”和“连上了但不对”就能分开归因，省掉大量来回猜测。
