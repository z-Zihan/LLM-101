...
这条边界还能用来解释一些日常现象。比如同一条 MCP 连接上，有的能力模型能自动选用，有的菜单只在界面出现——前者走了 Function Calling 的选择回路，后者根本没让模型参与。理解这一点之后，“为什么模型没有主动用到我装的工具”就有了第二个可能答案：除了描述写得不清楚，也可能是宿主应用压根把那项能力定位成人工操作，从未放进模型的候选列表。排查时先分清这两条通路，再谈优化描述。
