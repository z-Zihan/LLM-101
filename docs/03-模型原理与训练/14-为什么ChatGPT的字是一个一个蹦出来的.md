...
>
> LLM 最小原理闭环：**第 5 章 / 共 5 章** · [上一章：Loss 怎样修改参数](./12-没人给大模型批作业它怎么知道预测错了.md)
...
前四章已经用“中国的首都是北京”走过输入、预测、Attention 和训练。这一章让训练完成后的模型真正接收“中国的首都是”，选出“北京”，再继续生成。

看完这一篇，你应该能解释：Prefill 与 Decode 为什么不同，模型怎样选择并追加一个 Token，以及 KV Cache 为什么既加速又占用显存。
...
**试一试**：在支持流式输出的聊天产品里提一个需要长回答的问题，留意两段时间：按下发送后、第一个字出现前的等待，主要是排队和 Prefill 在工作；随后文字断续出现的节奏，就是 Decode 循环在逐 Token 前进。再看一眼打字过程中词块的长度变化，你会同时看到“生成单位是 Token”和“界面把若干 Token 合并显示”这两件事。
...
- 从第一章重走一遍：[文字怎样进入模型](../02-聊天Token与上下文/04-Token到底是什么.md)
...
- 检查自己能否完整复述：[LLM 最小原理闭环](../13-AI全景图/07-LLM最小原理闭环.md)
