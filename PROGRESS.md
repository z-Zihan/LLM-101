...
| 34 | [RAG 和 Fine-tuning 有什么区别](./docs/08-rag/11-rag-vs-finetuning.md) | Core | Done |
| 35 | [RAG 有哪些局限](./docs/08-rag/12-rag-limitations.md) | Core | Done |
| 36 | [MCP 是什么](./docs/09-mcp/01-what-is-mcp.md) | Core | Done |
...
## Batch 12：RAG vs Fine-tuning、RAG Limitations、MCP

完成：RAG 和 Fine-tuning、RAG 局限、MCP。
...
- 区分 RAG 的请求时证据与 Fine-tuning 的参数更新。
- 将 RAG 失败拆分为资料、检索、Context 和生成阶段。
- MCP 按 2026-07-28 官方规范核验 Host / Client / Server、能力与传输边界。
- 7 个来源与全部内部链接检查通过。

## Batch 13：MCP Client / Server、Primitives、MCP vs API

完成：MCP Client / Server、Tools / Resources / Prompts、MCP vs API。
...
- 合并页元数据统一为 Core / Main，不混写两个值。
- 移除发生异常重定向的 Specification Architecture 链接，使用官方 Architecture 文档与 Transports 规范。
...
## Batch 16：Agent vs Skill、AI Coding、Coding Agent

完成：Agent vs Skill、AI Coding、Coding Agent。

Review：Accuracy / Beginner / Architecture / Terminology / Duplication / Source / Link 全部 PASS。

Review 中修复：

- 区分 Agent 运行系统、Skill 能力包和 AI Coding 应用类别。
